    ...
    if [[ "$httpCode" -ne "404" ]];then echo "User is found"; echo "Test not past"; exit 1; else echo "User is not found"; fi
    if [[ -n "$json22" && -n "$json23" ]]; then echo ""; else echo "Input is empty"; echo "Test not past"; exit 1; fi
    if [[ "$json22" == "$json2" && "$json23" != "Resource not found" ]];then echo "Result is not as expected"; echo "$json22 should not be listed"; exit 1; 
    else echo "Test  past"; echo "$tests" > "$destdir"; fi